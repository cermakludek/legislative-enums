...
@DisplayName("LandTypeUseWebController Unit Tests")
class LandTypeUseWebControllerTest {
    ...
    @InjectMocks
    private LandTypeUseWebController controller;
    ...
    void setUp() {
        ...
        testDTO.setId(1L);
        testDTO.setLandTypeCode("LT001");
        testDTO.setLandUseCode("LU001");
    }
    ...
    void testList() {
        // Given
        List<LandTypeUseDTO> items = Arrays.asList(testDTO);
        ...
        // Then
        assertEquals("cuzk/land-type-uses/list", viewName);
    ...
    void testCreateForm() {
        ...
        // Then
        assertEquals("cuzk/land-type-uses/form", viewName);
        verify(model).addAttribute(eq("item"), any(LandTypeUseDTO.class));
    }
    ...
    void testCreateSuccess() {
        ...
        // Then
        assertEquals("redirect:/web/cuzk/land-type-uses", viewName);
    ...
    void testCreateException() {
        // Given
        when(bindingResult.hasErrors()).thenReturn(false);
        when(service.create(any(LandTypeUseDTO.class)))
        ...
        // Then
        assertEquals("redirect:/web/cuzk/land-type-uses/create", viewName);
        verify(service).create(testDTO);
    ...
    void testUpdateSuccess() {
        ...
        Long id = 1L;
        when(bindingResult.hasErrors()).thenReturn(false);
        when(service.update(eq(id), any(LandTypeUseDTO.class))).thenReturn(testDTO);
    ...
    void testUpdateException() {
        ...
        Long id = 1L;
        when(bindingResult.hasErrors()).thenReturn(false);
        when(service.update(eq(id), any(LandTypeUseDTO.class)))
        ...
        // Then
        assertEquals("redirect:/web/cuzk/land-type-uses/" + id + "/edit", viewName);
        verify(service).update(id, testDTO);
