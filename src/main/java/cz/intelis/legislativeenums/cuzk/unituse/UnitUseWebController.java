...
@Controller
@RequestMapping("/web/cuzk/unit-uses")
@RequiredArgsConstructor
public class UnitUseWebController {

    private final UnitUseService service;

    /**
     * Displays the list of unit uses.
     *
     * @param showAll if true, shows all records; if false (default), shows only currently valid
     * @param model the Spring MVC model
     * @return the view name for the unit uses list page
     */
    @GetMapping
    public String list(@RequestParam(required = false, defaultValue = "false") boolean showAll, Model model) {
        ...
        return "cuzk/unit-uses/list";
    }

    @GetMapping("/create")
    @PreAuthorize("hasAnyRole('ADMIN', 'CONTRIBUTOR')")
    public String createForm(Model model) {
        model.addAttribute("item", new UnitUseDTO());
        return "cuzk/unit-uses/form";
    }

    @PostMapping
    @PreAuthorize("hasAnyRole('ADMIN', 'CONTRIBUTOR')")
    public String create(@Valid @ModelAttribute("item") UnitUseDTO dto,
        ...
        try {
            ...
            return "redirect:/web/cuzk/unit-uses";
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("errorMessage", e.getMessage());
            return "redirect:/web/cuzk/unit-uses/create";
        }
    }
    ...
    public String update(@PathVariable Long id,
                        @Valid @ModelAttribute("item") UnitUseDTO dto,
                        BindingResult result,
                        RedirectAttributes redirectAttributes) {
        if (result.hasErrors()) {
            return "cuzk/unit-uses/form";
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("errorMessage", e.getMessage());
            return "redirect:/web/cuzk/unit-uses/" + id + "/edit";
        }
    }
